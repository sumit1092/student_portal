...
const renderTable =(data, tableId)=>{
    ...
    data.forEach(student => {
        ...
        row.innerHTML = 
           `<td>${student.id}</td>
            <td><img src="${student.avatar}" alt="pic" class="profile-pic"> ${student.first_name} ${student.last_name}</td>
...
const showTables =(showGenderTables)=> {
    document.getElementById('student-table').style.display = showGenderTables ? 'none' : 'table'
    document.getElementById('male-table').style.display = showGenderTables ? 'table' : 'none'
    document.getElementById('female-table').style.display = showGenderTables ? 'table' : 'none'
}
renderTable(students, "student-table")

const handleSearch =()=>{
    const searchValue = document.getElementById("search-input").value.toLowerCase()
    const filteredData = students.filter(student => student.first_name.toLowerCase().includes(searchValue) || 
        student.last_name.toLowerCase().includes(searchValue) || 
        student.email.toLowerCase().includes(searchValue)
    );
    renderTable(filteredData, "student-table")
}
...
function sortData(type) {
    let sortedData;
    switch (type) {
        case 'AtoZ': showTables(false)
            sortedData = [...students].sort((a, b) => (a.first_name + a.last_name).localeCompare(b.first_name + b.last_name))
            renderTable(sortedData, "student-table");
            break;
        case 'ZtoA': showTables(false)
            sortedData = [...students].sort((a, b) => (b.first_name + b.last_name).localeCompare(a.first_name + a.last_name))
            renderTable(sortedData, "student-table")
            break;
        case 'marks': showTables(false)
            sortedData = [...students].sort((a, b) => a.marks - b.marks);
            renderTable(sortedData, "student-table");
            break;
        case 'passing': showTables(false)
            sortedData = students.filter(student => student.passing)
            renderTable(sortedData, "student-table")
            break;
        case 'class': showTables(false)
            sortedData = [...students].sort((a, b) => a.class - b.class);
            renderTable(sortedData, "student-table");
            break;
        case 'gender': showTables(true)
            const males = students.filter(std => std.gender === 'Male')
            const females = students.filter(std => std.gender === 'Female')
            const others = ['Polygender', 'Agender', 'Genderqueer', 'Non-binary', 'Genderfluid', 'Bigender']
            const other = students.filter(std => others.includes(std.gender))
            renderTable(females, "female-table")
            renderTable(males, "male-table")
            renderTable(other, "other-table")
            break;
    }
}
